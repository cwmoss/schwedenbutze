...
class HotelDatepicker {
  constructor(input, options) {
    ...
    this.submitButtonName =
      this.submitButton && opts.submitButtonName ? opts.submitButtonName : "";
    this.i18n = opts.i18n || {
      selected: "Your stay:",
      night: "Night",
      nights: "Nights",
  ...
  createDatepickerDomString() {
    ...
    if (this.showTopbar) {
      ...
      if (buttonsHtml) {
        topBarHtml +=
          '<div class="datepicker__buttons">' + buttonsHtml + "</div>";
      }
      topBarHtml += "</div>";
    }
  ...
  createMonthDomString(_date) {
    ...
    for (let week = 0; week < 6; week++) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (extraText) {
          dayAttributes.class =
            dayAttributes.class + " datepicker__month-day--with-extra";
  ...
  getDayClasses(_day) {
    ...
    const isStartDate =
      this.getDateString(_day.time) === this.getDateString(this.startDate);
    ...
    if (_day.valid || _day.type === "visibleMonth") {
      ...
      if (this.disabledDaysOfWeek.length > 0) {
        if (
          this.disabledDaysOfWeek.indexOf(fecha.format(_day.time, "dddd")) > -1
        ) {
          _day.valid = false;
      ...
      if (this.noCheckInDaysOfWeek.length > 0) {
        if (
          this.noCheckInDaysOfWeek.indexOf(fecha.format(_day.time, "dddd")) > -1
        ) {
          isNoCheckIn = true;
          isFirstEnabledDate = false;
        }
      ...
      if (this.noCheckOutDaysOfWeek.length > 0) {
        if (
          this.noCheckOutDaysOfWeek.indexOf(fecha.format(_day.time, "dddd")) >
          -1
        ) {
          isNoCheckOut = true;
        }
  ...
  showSelectedDays() {
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      // Add selected class
      if (
        (this.start && this.end && this.end >= time && this.start <= time) ||
        (this.start &&
          !this.end &&
  ...
  dayClicked(day) {
    ...
    if (isSelectStart) {
      ...
      if (startDayEl) {
        if (
          this.hasClass(startDayEl, "datepicker__month-day--no-checkout") &&
          this.start > time
        ) {
          return;
        }
  ...
  updateSelectableRange() {
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      if (isSelecting) {
        if (
          this.hasClass(days[i], "datepicker__month-day--visibleMonth") &&
        ...
        this.hasClass(days[i], "datepicker__month-day--checkout-enabled") ||
        this.hasClass(days[i], "datepicker__month-day--before-disabled-date")
      ) {
      ...
      if (this.start && !this.end) {
        ...
        if (this.hoveringTooltip) {
          if (typeof this.hoveringTooltip === "function") {
            ...
            const label =
              nights === 1 ? this.lang("night") : this.lang("nights");
